...
$('.go').click(function(){
	...
	if (url == ''){
		...
		// Call my API
		$.getJSON("https://cors-anywhere.herokuapp.com/https://www.reviewsmaker.com/api/public/caring?url=" + url, function (data){
			...
			$.each( data.reviews, function( key, value ) {
			  ...
			  $('.results').append('<b>Review Timestamp: </b>' + value.timestamp + "<br>");
			  $('.results').append('<b>Review Rating: </b>' + value.rating + "<br>");
			  $('.results').append('<b>Review Text: </b>' + value.review + "<hr>");
			});
